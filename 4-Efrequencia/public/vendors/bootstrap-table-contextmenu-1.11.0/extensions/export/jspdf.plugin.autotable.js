    ...
    var defaultOptions = function () {
        return {
            ...
            renderHeader: function (doc, pageNumber, settings) {
               doc.text((new Date().toLocaleString()),doc.internal.pageSize.width - 100,20);
               doc.text(('JUSTIÇA FEDERAL DE PRIMERIO GRAU NO AMAZONAS'),20,20);
               doc.text(('SEÇÃO DE CADASTRO DE PESSOAL-SECAP/NUCRE/SECAD/DIREF'),20,30);
               doc.text(('RELATÓRIO DE PEDIDOS ('+$('#select-tipo-pedido option:selected').text()+') DO PERÍODO DE '+$('#input-filtro-data-inicio').val()+" A "+$('#input-filtro-data-fim').val()),20,40);
            },
            renderFooter: function (doc, lastCellPos, pageNumber, settings) {
                doc.text(('Página ' + pageNumber),20,doc.internal.pageSize.height -10);
            },
            ...
            overflowColumns: [2,3,6,7,8], // Specify which colums that gets subjected to the overflow method chosen. false indicates all
